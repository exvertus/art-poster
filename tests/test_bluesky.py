...
def test_img_post(test_client, random_qr_code):
    original_number = random_qr_code["number"]
    image = random_qr_code["image"]
    image_bytes = random_qr_code["bytes"]

    post_response = bluesky.img_post(image, 
                                     image_bytes, 
                                     caption='This is a test.',
                                     client=test_client)

    post_rkey = post_response.uri.split('/')[-1]
    get_response = test_client.get_post(post_rkey)
    ...
    decoded_number = decode_qr_from_bytes(blob_bytes)
